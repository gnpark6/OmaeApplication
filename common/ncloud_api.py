...
if __name__ == '__main__':
    ...
    access_key = params['NC_ACCESS_KEY_ID']
    secret_key = params['NC_SECRET_KEY']


    if sys.argv[1] == 'm':
        today = datetime.today()
        sale_date = f"{today.year}년 {today.month}월 {today.day}일"

        toaddress = ["mhshin71@gmail.com", "mhshin@justpayments.co.kr"]
        customer_name = ["신길동", "성수동"]
        store_name = ["신길 호프", "성동 커피"]

        sender = "just-admin@justpayments.co.kr"
        title = "${customer_name}님 반갑습니다. "
        body = "귀하의 매장 '${store_name}'의 ${sale_date} 팩토링 준비가 완료되었습니다."
        individual = True
        advertising = False
        ...
        for x in range(2):
            message = {
                "address": toaddress[x],
                "name": customer_name[x],
                "type": "R",
                "parameters": {
                    "customer_name": customer_name[x],
                    "store_name": store_name[x],
                    "sale_date": sale_date
                }
            }
        ...
        ms = MailSender('', access_key, secret_key)
        retcode = ms.request_sending(sender, title, body, recipients, individual, advertising)
        print(retcode)

    elif sys.argv[1] == 's':

        service_id = params['NC_SMS_SERVICE_ID']
        calling_number = params['NC_SMS_CALLING_NUMBER']

        sms = SMSSender(access_key, secret_key, service_id, calling_number)
        ...
        content = f'[오매] 인증번호: {str}\n타인 유출 주의'
        recipients = {
            'to': '01023749127'
        }

        retcode = sms.request_sending('82', content, [recipients])
        print(retcode)
